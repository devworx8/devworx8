 ...
 *
 * Hook to read/write per-category notification preferences.
 * Auto-creates a default row on first access (upsert pattern).
 *
...
export function useNotificationPreferences(): UseNotificationPreferencesReturn {
  ...
  const fetchPrefs = useCallback(async () => {
    ...
    try {
      ...
      // If no data, default prefs are already set
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Failed to load preferences';
      logger.error('[useNotificationPreferences]', err);
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const savePrefs = useCallback(async () => {
    ...
    try {
      ...
      logger.debug('[useNotificationPreferences] Saved successfully');
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Failed to save preferences';
      logger.error('[useNotificationPreferences] Save failed:', err);
      setError(msg);
    } finally {
      setSaving(false);
    }
  }, [user?.id, supabase, prefs]);
