...
export function isWeeklyMenuBridgeEnabled(): boolean {
  return envFlag(
    process.env.NEXT_PUBLIC_WEEKLY_MENU_BRIDGE_ENABLED || process.env.EXPO_PUBLIC_WEEKLY_MENU_BRIDGE_ENABLED,
    true,
  );
}
...
export function isWeeklyMenuDedicatedEnabled(): boolean {
  return envFlag(
    process.env.NEXT_PUBLIC_WEEKLY_MENU_DEDICATED_ENABLED || process.env.EXPO_PUBLIC_WEEKLY_MENU_DEDICATED_ENABLED,
    false,
  );
}
