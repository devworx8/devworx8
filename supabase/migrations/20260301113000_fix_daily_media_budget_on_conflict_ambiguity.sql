-- Fix daily media budget RPC runtime failure:
-- ERROR: column reference "usage_date" is ambiguous
-- Cause: RETURNS TABLE includes `usage_date`, which conflicts with ON CONFLICT
-- target list `ON CONFLICT (user_id, usage_date, feature)`.
-- Fix: use explicit PK constraint target instead.
...
BEGIN
  ...
  ON CONFLICT ON CONSTRAINT daily_media_usage_pkey DO NOTHING;
  ...
  IF v_feature = 'auto_scan' THEN
    ...
    WHERE u.user_id = v_user_id AND u.usage_date = v_date AND u.feature = v_feature;
  ELSE
  ...
  RETURN QUERY SELECT v_feature, v_date, v_used, v_limit,
...
BEGIN
  ...
  IF v_limit >= 0 AND v_used + v_amount > v_limit THEN
    RETURN QUERY SELECT false, v_feature, v_date, v_used, v_limit, greatest(0, v_limit - v_used);
    RETURN;
  END IF;
  ...
  RETURN QUERY SELECT true, v_feature, v_date, coalesce(v_used, 0), v_limit,
