-- -----------------------------------------------------------------------------
-- Finance P2: Family credits lifecycle, petty-cash dedup, monthly fee cron
--
-- 1. Petty-cash ↔ financial_transactions dedup FK
-- 2. get_finance_month_snapshot: exclude double-counted expenses
-- 3. apply_family_credit RPC
-- 4. approve_pop_payment: auto-apply credits + principal notification
-- 5. pg_cron schedule for generate-monthly-fees Edge Function
-- -----------------------------------------------------------------------------

-- ═══════════════════════════════════════════════════════════════════════════════
-- 1. PETTY CASH DEDUP FOREIGN KEY
-- ═══════════════════════════════════════════════════════════════════════════════
-- Some organisations record the same expense in both petty_cash_transactions
-- AND financial_transactions. This FK column lets us link them so the snapshot
-- function can avoid double-counting.
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Only change from P0: the financial_transactions expense query now excludes
-- rows that are already accounted for via petty_cash_transactions.financial_transaction_id.
...
DECLARE
  ...
  v_family_credits_available numeric(12,2) := 0;
BEGIN
  ...
  -- ── Excluded inactive students (reconciliation) ──────────────────────────
  ...
  -- ── Expenses: financial transactions (DEDUP: exclude rows linked from petty cash) ──
  BEGIN
    SELECT
    ...
    WHERE ft.preschool_id = p_org_id
      ...
      -- P2 Dedup: exclude financial_transactions that are already reflected
      -- in petty_cash_transactions via the linking FK.
      AND ft.id NOT IN (
        SELECT pct.financial_transaction_id
        FROM public.petty_cash_transactions pct
        WHERE pct.financial_transaction_id IS NOT NULL
      );
  EXCEPTION WHEN undefined_table OR undefined_column THEN
  ...
  -- ── Available family credits (new) ───────────────────────────────────────
  BEGIN
    SELECT coalesce(sum(fc.remaining_amount), 0)
    INTO v_family_credits_available
    FROM public.family_credits fc
    WHERE fc.preschool_id = p_org_id
      AND fc.status = 'available'
      AND fc.remaining_amount > 0;
  EXCEPTION WHEN undefined_table OR undefined_column THEN
    v_family_credits_available := 0;
  END;
  ...
  RETURN jsonb_build_object(
    ...
    'family_credits_available', round(coalesce(v_family_credits_available, 0), 2),
...
-- 3. apply_family_credit RPC
-- ═══════════════════════════════════════════════════════════════════════════════
-- Atomically apply a family credit against an outstanding student fee.
-- Decrements credit remaining_amount, creates payment_allocation, updates fee.
...
DECLARE
  ...
  v_actor_org uuid;
  v_is_super boolean := false;
  v_credit public.family_credits%ROWTYPE;
  v_fee record;
  v_fee_amount numeric(12,2);
  v_fee_outstanding numeric(12,2);
  v_apply_amount numeric(12,2);
  v_new_remaining numeric(12,2);
  v_new_fee_paid numeric(12,2);
  v_new_fee_outstanding numeric(12,2);
  v_new_fee_status text;
BEGIN
  IF v_actor IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Not authenticated');
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'Family credit not found');
  END IF;

  IF NOT v_is_super AND v_actor_org IS DISTINCT FROM v_credit.preschool_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'Cross-organization access denied');
  END IF;

  IF v_credit.status <> 'available' THEN
    RETURN jsonb_build_object('success', false, 'error', 'Credit is not available (status: ' || v_credit.status || ')');
  END IF;

  v_apply_amount := round(least(p_amount, v_credit.remaining_amount), 2);
  IF v_apply_amount <= 0 THEN
    RETURN jsonb_build_object('success', false, 'error', 'Amount must be greater than zero');
  END IF;

  -- Lock and validate fee
  SELECT sf.id,
         sf.student_id,
         sf.fee_structure_id,
         sf.final_amount,
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'Student fee not found');
  END IF;

  -- Verify the fee belongs to a student in the same org
  IF NOT EXISTS (
    SELECT 1 FROM public.students s
    WHERE s.id = v_fee.student_id
      AND COALESCE(s.organization_id, s.preschool_id) = v_credit.preschool_id
  ) THEN
    RETURN jsonb_build_object('success', false, 'error', 'Fee does not belong to the same organisation');
  END IF;

  v_fee_amount := round(coalesce(v_fee.final_amount, v_fee.amount, 0), 2);
  v_fee_outstanding := round(greatest(coalesce(v_fee.amount_outstanding, v_fee_amount - coalesce(v_fee.amount_paid, 0), v_fee_amount), 0), 2);

  IF v_fee_outstanding <= 0 THEN
    RETURN jsonb_build_object('success', false, 'error', 'Fee is already fully paid');
  END IF;

  -- Cap the apply amount to what the fee actually needs
  v_apply_amount := round(least(v_apply_amount, v_fee_outstanding), 2);

  -- Create payment allocation using the origin payment
  IF v_credit.origin_payment_id IS NOT NULL THEN
    ...
    VALUES (
      v_credit.origin_payment_id,
      p_student_fee_id,
      v_apply_amount,
      coalesce(v_fee.billing_month, date_trunc('month', current_date)::date),
      coalesce(v_fee.category_code, v_credit.category_code, 'tuition'),
      coalesce(p_notes, 'Applied from family credit ' || p_credit_id::text),
      v_actor,
      now()
    )
    ...
    DO UPDATE SET
      ...
      notes = coalesce(EXCLUDED.notes, payment_allocations.notes),
      updated_at = now();
  END IF;

  -- Update fee
  v_new_fee_paid := round(coalesce(v_fee.amount_paid, 0) + v_apply_amount, 2);
  v_new_fee_outstanding := round(greatest(v_fee_amount - v_new_fee_paid, 0), 2);
  v_new_fee_status := CASE
    WHEN v_new_fee_outstanding = 0 THEN 'paid'
    WHEN v_new_fee_paid > 0 THEN 'partially_paid'
    ELSE coalesce(v_fee.status, 'pending')
  END;

  UPDATE public.student_fees
  SET amount_paid = v_new_fee_paid,
      amount_outstanding = v_new_fee_outstanding,
      status = v_new_fee_status,
      paid_date = CASE WHEN v_new_fee_outstanding = 0 THEN coalesce(paid_date, current_date) ELSE paid_date END,
      updated_at = now()
  WHERE id = p_student_fee_id;

  -- Update credit
  v_new_remaining := round(v_credit.remaining_amount - v_apply_amount, 2);

  UPDATE public.family_credits
  SET remaining_amount = v_new_remaining,
      status = CASE WHEN v_new_remaining <= 0 THEN 'applied' ELSE 'available' END,
      metadata = metadata || jsonb_build_object(
        'last_applied_at', now()::text,
        'last_applied_to_fee', p_student_fee_id::text,
        'last_applied_amount', v_apply_amount
      ),
      updated_at = now()
  WHERE id = p_credit_id;
  ...
  RETURN jsonb_build_object(
    ...
    'credit_remaining', v_new_remaining,
    'credit_status', CASE WHEN v_new_remaining <= 0 THEN 'applied' ELSE 'available' END,
    'fee_paid', v_new_fee_paid,
    'fee_outstanding', v_new_fee_outstanding,
    'fee_status', v_new_fee_status
  );
END;
$$;
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Changes from P0 version:
-- a) After creating a family credit from overpayment, auto-apply ALL available
--    credits for this parent+org against outstanding fees for the target month.
-- b) Insert an in_app_notification for the school principal(s) when credits
--    are auto-applied.
...
DECLARE
  ...
  -- P2 additions: credit auto-apply
  v_credit_rec record;
  v_credit_apply numeric(12,2);
  v_credit_total_applied numeric(12,2) := 0;
  v_credit_fees_applied int := 0;
  v_target_month date;
  v_outstanding_fee record;
  v_principal_id uuid;
BEGIN
  IF v_actor IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Not authenticated');
  ...
  SELECT
    ...
    coalesce(s.first_name || ' ' || s.last_name, s.first_name, 'Student')
  INTO
    v_student_is_active,
    v_student_status,
    v_student_org,
  ...
  -- ── Overpayment → family credit ─────────────────────────────────────────
  v_overpayment := round(greatest(v_payment_amount - v_total_alloc, 0), 2);
  v_target_month := (v_billing_month + INTERVAL '1 month')::date;
  ...
  IF v_overpayment > 0 THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'pop_upload_id', v_upload.id,
        'billing_month', v_billing_month::text,
        'auto_apply_target_month', v_target_month::text
      ),
      now(),
      now()
    );
  ...
  -- ═══════════════════════════════════════════════════════════════════════════
  -- P2 CREDIT AUTO-APPLY
  -- After creating any new credit (overpayment), try to auto-apply ALL
  -- available credits for this parent+org to outstanding fees for the
  -- target month (next month after billing_month).
  -- ═══════════════════════════════════════════════════════════════════════════
  BEGIN
    FOR v_credit_rec IN
      SELECT fc.id,
             fc.remaining_amount,
             fc.origin_payment_id,
             fc.category_code AS credit_category
      FROM public.family_credits fc
      WHERE fc.parent_id = v_upload.uploaded_by
        AND fc.preschool_id = v_upload.preschool_id
        AND fc.status = 'available'
        AND fc.remaining_amount > 0
      ORDER BY fc.created_at ASC
      FOR UPDATE
    LOOP
      -- Find outstanding fees for target month for this student
      FOR v_outstanding_fee IN
        SELECT sf.id AS fee_id,
               round(greatest(
                 coalesce(sf.amount_outstanding,
                   coalesce(sf.final_amount, sf.amount, 0) - coalesce(sf.amount_paid, 0)),
                 0
               ), 2) AS fee_outstanding,
               sf.billing_month AS fee_billing_month,
               sf.category_code AS fee_category
        FROM public.student_fees sf
        WHERE sf.student_id = v_upload.student_id
          AND sf.billing_month = v_target_month
          AND lower(coalesce(sf.status, '')) NOT IN ('paid', 'waived')
          AND round(greatest(
                coalesce(sf.amount_outstanding,
                  coalesce(sf.final_amount, sf.amount, 0) - coalesce(sf.amount_paid, 0)),
                0
              ), 2) > 0
        ORDER BY sf.due_date ASC NULLS LAST, sf.created_at ASC
        FOR UPDATE
      LOOP
        IF v_credit_rec.remaining_amount <= 0 THEN
          EXIT; -- credit exhausted
        END IF;

        v_credit_apply := round(least(v_credit_rec.remaining_amount, v_outstanding_fee.fee_outstanding), 2);

        IF v_credit_apply <= 0 THEN
          CONTINUE;
        END IF;

        -- Create allocation if we have an origin payment
        IF v_credit_rec.origin_payment_id IS NOT NULL THEN
          INSERT INTO public.payment_allocations (
            payment_id,
            student_fee_id,
            allocated_amount,
          ...
          VALUES (
            v_credit_rec.origin_payment_id,
            v_outstanding_fee.fee_id,
            v_credit_apply,
            v_outstanding_fee.fee_billing_month,
            coalesce(v_outstanding_fee.fee_category, v_credit_rec.credit_category, 'tuition'),
            'Auto-applied from family credit',
            v_actor,
            now()
          )
          ON CONFLICT (payment_id, student_fee_id)
          DO UPDATE SET
            allocated_amount = payment_allocations.allocated_amount + EXCLUDED.allocated_amount,
            notes = 'Auto-applied from family credit',
            updated_at = now();
        END IF;

        -- Update the fee
        UPDATE public.student_fees
        SET amount_paid = round(coalesce(amount_paid, 0) + v_credit_apply, 2),
            amount_outstanding = round(greatest(
              coalesce(final_amount, amount, 0) - (coalesce(amount_paid, 0) + v_credit_apply),
              0
            ), 2),
            status = CASE
              WHEN round(greatest(
              ...
              WHEN (coalesce(amount_paid, 0) + v_credit_apply) > 0 THEN 'partially_paid'
              ELSE status
            END,
            paid_date = CASE
              WHEN round(greatest(
                coalesce(final_amount, amount, 0) - (coalesce(amount_paid, 0) + v_credit_apply),
                0
              ), 2) = 0 THEN coalesce(paid_date, current_date)
              ELSE paid_date
            END,
            updated_at = now()
        WHERE id = v_outstanding_fee.fee_id;

        -- Update credit remaining
        v_credit_rec.remaining_amount := round(v_credit_rec.remaining_amount - v_credit_apply, 2);

        UPDATE public.family_credits
        SET remaining_amount = v_credit_rec.remaining_amount,
            status = CASE WHEN v_credit_rec.remaining_amount <= 0 THEN 'applied' ELSE 'available' END,
            metadata = metadata || jsonb_build_object(
              'auto_applied_at', now()::text,
              'auto_applied_to_fee', v_outstanding_fee.fee_id::text,
              'auto_applied_amount', v_credit_apply
            ),
            updated_at = now()
        WHERE id = v_credit_rec.id;

        v_credit_total_applied := round(v_credit_total_applied + v_credit_apply, 2);
        v_credit_fees_applied := v_credit_fees_applied + 1;
      END LOOP;
    END LOOP;

    -- Notify principal(s) if any credits were auto-applied
    IF v_credit_total_applied > 0 THEN
      FOR v_principal_id IN
        SELECT p.id
        FROM public.profiles p
        WHERE COALESCE(p.organization_id, p.preschool_id) = v_upload.preschool_id
          AND lower(p.role) IN ('principal', 'principal_admin', 'admin')
        LIMIT 5
      LOOP
        BEGIN
          ...
          VALUES (
            v_principal_id,
            'Family Credit Auto-Applied',
            format(
              'R%.2s in family credits were auto-applied to %s outstanding fees for %s (%s).',
              v_credit_total_applied::text,
              v_credit_fees_applied::text,
              v_student_name,
              to_char(v_target_month, 'Mon YYYY')
            ),
            'finance',
            jsonb_build_object(
              'action', 'credit_auto_applied',
              'student_id', v_upload.student_id,
              'student_name', v_student_name,
              'parent_id', v_upload.uploaded_by,
              'preschool_id', v_upload.preschool_id,
              'total_applied', v_credit_total_applied,
              'fees_applied', v_credit_fees_applied,
              'target_month', v_target_month::text,
        ...
        EXCEPTION WHEN OTHERS THEN
          -- Non-fatal: notification insert failure should not abort payment
          NULL;
        END;
      END LOOP;
    END IF;
  EXCEPTION WHEN undefined_table OR undefined_column THEN
    -- family_credits or in_app_notifications table may not exist yet
    NULL;
  END;

  -- ── Update POP upload status ─────────────────────────────────────────────
  UPDATE public.pop_uploads
  SET status = 'approved',
      reviewed_by = v_actor,
      reviewed_at = now(),
  ...
  SET fee_ids = CASE WHEN coalesce(array_length(v_fee_id_list, 1), 0) > 0 THEN v_fee_id_list ELSE fee_ids END,
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
        ...
        'credits_auto_applied', v_credit_total_applied,
        'credits_fees_applied', v_credit_fees_applied
      ),
      updated_at = now()
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Fires on the 1st of each month at 00:05 UTC (5 mins past midnight gives
-- the system time to settle after any month-end jobs).

DO $$
BEGIN
  -- pg_cron + pg_net required for HTTP calls from cron
...
BEGIN
  ...
  IF v_url IS NOT NULL AND v_cron_secret IS NOT NULL THEN
    PERFORM cron.schedule(
      ...
      format(
        ...
        SELECT net.http_post(
          url := '%s/functions/v1/generate-monthly-fees',
    ...
    RAISE NOTICE 'pg_cron: generate-monthly-fees scheduled for 00:05 UTC on 1st of each month';
  ELSE
    RAISE NOTICE 'pg_cron: skipped — SUPABASE_URL or CRON_SECRET not set in app.settings';
  END IF;
EXCEPTION WHEN OTHERS THEN
