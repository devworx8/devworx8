-- Migration: Harden rpc_assign_teacher_seat against remaining 409 conflict paths
-- Problem:  Legacy split rows can exist in public.users where one row matches id
--           and another matches auth_user_id. A non-deterministic LIMIT 1 select
--           can update the wrong row and trigger unique-constraint 409 errors.
-- Fix:      Prefer auth_user_id row deterministically, fallback to id row, and
--           make seat insert idempotent with ON CONFLICT DO NOTHING.
  ...
  IF v_target_user_db_id IS NULL THEN
    ...
    EXCEPTION
      WHEN unique_violation THEN
        -- Another row already owns auth_user_id (or was inserted concurrently).
        -- Continue with deterministic lookup + update path below.
        NULL;
    END;
