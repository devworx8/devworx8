-- Migration: Fix rpc_assign_teacher_seat 400 when profile has wrong role/school
-- Problem:   When a teacher is in the `teachers` table but their `profiles` row
--            still has role='parent' and preschool_id=NULL (common when a parent
--            is later added as staff), the function finds the profile first, sees
--            the wrong role, and raises "Target must be school staff" (HTTP 400).
--            The teachers-table fallback only fires when NO profile is found.
-- Fix:       After the first profile lookup, if the role/school doesn't match
--            staff requirements, also attempt the teachers-table fallback. If
--            confirmed as staff there, promote the profile role + school.
-- Scope:     DROP + CREATE rpc_assign_teacher_seat (same signature, SECURITY DEFINER)
