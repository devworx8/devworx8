...
BEGIN
  ...
  IF has_org_members AND has_member_type THEN
    extra_clause := 'OR EXISTS (
  ...
  IF NOT EXISTS (
    ...
    EXECUTE format(
      'CREATE POLICY "Admins can delete invite codes" ON public.join_requests
       FOR DELETE TO authenticated
       USING (invited_by = auth.uid() %s)',
      extra_clause
    );
  END IF;
END $$;
