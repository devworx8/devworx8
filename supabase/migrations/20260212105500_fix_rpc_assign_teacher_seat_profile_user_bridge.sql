-- -----------------------------------------------------------------------------
-- Fix teacher seat assignment when profile exists but users row is missing.
-- Also resolves target teacher by profile.id or auth_user_id for compatibility.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Verify target teacher + same school
  IF v_target_profile_id IS NULL
     OR v_target_profile_role <> 'teacher'
  ...
  IF v_target_auth_user_id IS NULL THEN
    RAISE EXCEPTION 'Target teacher account is not linked to auth user';
  END IF;
  ...
  -- Auto-provision missing users row from profile
  IF v_target_user_db_id IS NULL THEN
    IF v_target_email IS NULL OR v_target_email = '' THEN
      v_target_email := format('teacher-%s@placeholder.local', replace(v_target_auth_user_id::text, '-', ''));
    END IF;
    ...
    -- `users` table has an audit trigger that writes `audit_logs.user_id = auth.uid()`,
    -- but `audit_logs.user_id` references `users.id` (not auth user ids).
    -- Clear auth uid for this insert so the audit row stores NULL instead of FK-invalid value.
    ...
    INSERT INTO public.users (
      ...
      v_target_auth_user_id,
      v_target_email,
      COALESCE(v_target_name, 'Teacher'),
      'teacher',
      v_school,
      true,
      NOW(),
    ...
    PERFORM set_config('request.jwt.claim.sub', COALESCE(v_caller_auth_uid::text, ''), true);
  ...
  IF v_assigned_by_db_id IS NULL THEN
    SELECT id INTO v_assigned_by_db_id
    FROM public.users
    WHERE id = v_caller_auth_uid;
  END IF;
