...
DECLARE
  ...
  v_old jsonb := '{}'::jsonb;
  v_new jsonb := '{}'::jsonb;
  v_allowed_org_types text[] := ARRAY['preschool', 'daycare', 'primary_school', 'k12', 'skills', 'tertiary', 'org', 'other'];
  v_allowed_school_types text[] := ARRAY['preschool', 'combined', 'community_school', 'primary', 'secondary', 'k12'];
BEGIN
  ...
  IF v_next_type = '' THEN
    RAISE EXCEPTION 'Next type cannot be empty';
  END IF;

  IF v_entity_type = 'school' THEN
    RAISE EXCEPTION 'School entity type is fixed in this flow';
  END IF;
  ...
  IF v_entity_type = 'organization' THEN
    ...
    v_legacy_type := CASE
      ...
      WHEN v_next_type = 'skills' THEN 'skills'
