-- MINIMAL CORE BUSINESS TABLES MIGRATION
...
-- Basic RLS policies
CREATE POLICY IF NOT EXISTS billing_plans_public_read 
  ON billing_plans FOR SELECT USING (active = true);

CREATE POLICY IF NOT EXISTS homework_assignments_tenant_isolation 
  ON homework_assignments USING (preschool_id = current_preschool_id());

CREATE POLICY IF NOT EXISTS lessons_tenant_isolation 
  ON lessons USING (preschool_id = current_preschool_id());

CREATE POLICY IF NOT EXISTS ai_generations_tenant_isolation 
  ON ai_generations USING (preschool_id = current_preschool_id());

CREATE POLICY IF NOT EXISTS config_kv_public_read 
...
VALUES (
  'migration_minimal_core_business_tables',
  '{"version": "1.0.0", "completed_at": "' || now()::text || '", "tables_added": 5}',
  'Minimal core business tables migration completion log',
  false
